...
{
       0,   119,   119,   120,   123,   129,   133,   159,   160,   163,
     166,   174,   178,   186,   190,   194,   199,   216,   235,   235,
     244
};
#endif
...
#line 199 "src/parser/parser.y"
    ...
    http_method_t method_type = UNIMPLEMENTED;
    if (strcmp("GET", (yyvsp[(1) - (6)].str)) == 0) {
        method_type = GET;
    } else if (strcmp("HEAD", (yyvsp[(1) - (6)].str)) == 0) {
        method_type = HEAD;
    } else if (strcmp("POST", (yyvsp[(1) - (6)].str)) == 0) {
        method_type = POST;
    }

    parsing_request->method_type = method_type;
...
#line 216 "src/parser/parser.y"
    ...
    int capacity = parsing_request->c_headers;
        YPRINTF("capacity: %d, existing: %d\n", capacity, parsing_request->n_headers);

    if (capacity == parsing_request->n_headers) {
        YPRINTF("resizing...\n");
        capacity *= 2;
        parsing_request->headers = (RequestHeader *) realloc(parsing_request->headers,
                                                             capacity * sizeof(*parsing_request->headers));
        parsing_request->c_headers = capacity;
    }
    YPRINTF("After resizing... %d\n", parsing_request->c_headers);
    strcpy(parsing_request->headers[parsing_request->n_headers].key, (yyvsp[(1) - (7)].str));
	strcpy(parsing_request->headers[parsing_request->n_headers].value, (yyvsp[(5) - (7)].str));
	parsing_request->n_headers++;
}
    break;
