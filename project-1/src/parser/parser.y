...
request_line: token t_sp text t_sp text t_crlf {
    ...
    if (strcmp("GET", $1) == 0) {
        method_type = GET;
    } else if (strcmp("HEAD", $1) == 0) {
        method_type = HEAD;
    } else if (strcmp("POST", $1) == 0) {
        method_type = POST;
    }
...
request_header: token ows t_colon ows text ows t_crlf {
    ...
    strcpy(parsing_request->headers[parsing_request->n_headers].key, $1);
	strcpy(parsing_request->headers[parsing_request->n_headers].value, $5);
